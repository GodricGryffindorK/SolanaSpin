...
function Admin() {
  ...
  const updateSettingInfo = async () => {
    try {
      ...
      if (sData) {
        ...
        const pay_amount = sData?.price?.toNumber() / (10 ** REWARD_TOKEN_DECIMAL);
  ...
  const onSetRows = async (e) => {
    ...
    const result = await setAdminInfos(wallet, connection, itemInfos);
    if (result)
      NotificationManager.success("Success on setting items");
    else
      NotificationManager.error("Error on setting items");
  };
  ...
  const onSettingInfo = async () => {
    ...
    const result = await setPayInfo(wallet, connection, fronkAmount, devFee, devWallet, burnFee, burnWallet);
    if (result) NotificationManager.success('Success on setting pay info');
    else NotificationManager.error('Error in setting pay info');
  ...
  return isAdmin == 2 ? (
    ...
    <div className="admin">
      ...
      >
        ...
        <div className="">
          <label htmlFor="">Fronk Amount</label>
          <input
